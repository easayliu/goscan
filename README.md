...
`Ctrl-C` / `SIGTERM` 是优雅退出：不再接新任务（`POST /sync` 回 503，到点的 cron 跳过），
在跑的同步和 `DELETE /tasks/{id}` 一样停在两趟之间 —— 手上这一趟写完再退，最多等 10 分钟，
结果是 `cancelled`，没跑的趟列在 `not_run`。这一趟的账期已经清空，半路掐断就只剩半个账期，
所以 Deployment 的 `terminationGracePeriodSeconds` 是 660 秒；空闲时进程立刻退出。
