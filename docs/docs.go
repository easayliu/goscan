...
const docTemplate = `{
    ...
    "paths": {
        ...
        "/sync": {
            ...
            "post": {
                "description": "Immediately trigger billing data sync operation for specified cloud provider. Supports multiple cloud providers like Volcengine, Alibaba Cloud, etc.\nSupports configuring sync mode, time range, granularity and other parameters.\nFor AliCloud the granularity field picks the target table: monthly writes alicloud_bill_monthly, daily writes alicloud_bill_daily, both writes each. When it is empty the bill_period format decides instead (YYYY-MM monthly, YYYY-MM-DD daily). VolcEngine has a single table and ignores granularity.\nThe endpoint returns as soon as the task is registered; poll GET /tasks/{task_id} for the outcome.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                ...
                "responses": {
                    "200": {
                        "description": "Sync task accepted, poll the returned task_id",
                        "schema": {
                            "$ref": "#/definitions/models.SyncTriggerResponse"
                        }
                    },
                    "400": {
                    ...
                    "409": {
                        "description": "A sync for this provider is already running",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    "429": {
                        "description": "Too many tasks in flight, retry later",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    },
                    ...
                    "503": {
                        "description": "Shutting down, no new tasks are taken; retry later",
                        "schema": {
                            "$ref": "#/definitions/models.ErrorResponse"
                        }
                    }
    ...
    "definitions": {
        ...
        "models.ErrorResponse": {
            ...
            "properties": {
                ...
                "details": {
                    "type": "string",
                    "example": "Provider field cannot be empty"
                },
                "error": {
                    "type": "boolean",
                    "example": true
        ...
        "models.JobRequest": {
            ...
            "properties": {
                ...
                "description": {
                    "type": "string",
                    "example": "Daily VolcEngine billing sync at 2 AM"
                },
                "enabled": {
                    "type": "boolean",
                    "example": true
        ...
        "models.SyncTriggerRequest": {
            ...
            "properties": {
                "bill_period": {
                    "description": "A single billing period. The format also picks the granularity when the\nfield above is empty: YYYY-MM is monthly, YYYY-MM-DD is one day.",
                    "type": "string",
                    "example": "2026-09"
                },
                "create_table": {
                    "description": "Create the target tables first if they are missing.",
                    "type": "boolean",
                    "example": false
                },
                "end_period": {
                    "description": "End of the inclusive period range.",
                    "type": "string",
                    "example": "2026-06"
                },
                "force_update": {
                    "description": "Re-fetch periods that already have data instead of skipping them.",
                    "type": "boolean",
                    "example": false
                },
                "granularity": {
                    "description": "AliCloud only, selects the target table: monthly writes alicloud_bill_monthly,\ndaily writes alicloud_bill_daily, both writes each in one run. Ignored for\nVolcEngine, whose bills land in the single volcengine_bill table.",
                    "type": "string",
                    "enum": [
                        "monthly",
                        "daily",
                ...
                "limit": {
                    "description": "Cap on records to sync; 0 means no cap.",
                    "type": "integer",
                    "example": 0
                },
                "provider": {
                    "description": "Which cloud to sync. Required.",
                    "type": "string",
                    "enum": [
                        "volcengine",
                        "alicloud"
                ...
                "start_period": {
                    "description": "Start of an inclusive period range, used instead of bill_period to backfill\nseveral periods in one task.",
                    "type": "string",
                    "example": "2026-01"
                },
                "sync_mode": {
                    "description": "standard fetches every period in full, sync-optimal compares record counts\nand only fills the gap. Empty uses the provider default from the config.",
                    "type": "string",
                    "enum": [
                        "standard",
                        "sync-optimal"
                    ],
                    "example": "sync-optimal"
                },
                "use_distributed": {
                    "description": "Write through the Distributed table instead of the local one.",
                    "type": "boolean",
                    "example": false
                }
            }
        },
        "models.SyncTriggerResponse": {
            "type": "object",
            "properties": {
                "message": {
                    "type": "string",
                    "example": "Sync triggered for provider alicloud"
                },
                "provider": {
                    "type": "string",
                    "example": "alicloud"
                ...
                "task_id": {
                    "type": "string",
                    "example": "6f1c8f0e-6c2a-4a1f-9a4d-1f0b7a2c3d4e"
                },
                "timestamp": {
                    "type": "string",
                    "example": "2026-09-11T08:13:24Z"
                }
            }
        },
        "models.SystemStatus": {
        ...
        "models.TaskRequest": {
            ...
            "properties": {
                "description": {
                    "type": "string",
                    "example": "Sync VolcEngine billing data"
                },
                "name": {
                    "type": "string",
        ...
        "models.WeChatNotificationRequest": {
            ...
            "properties": {
                "alert_threshold": {
                    "description": "Cost change alert threshold (percentage), triggers alert when cost change exceeds this value, defaults to system configuration value, range: 0-100",
                    "type": "number",
                    "maximum": 100,
                    "minimum": 0,
                    "example": 10
                ...
                "date": {
                    "description": "Analysis date, format: YYYY-MM-DD, defaults to current date. Example: 2025-09-12",
                    "type": "string",
                    "example": "2025-09-12"
                },
                "force_notify": {
                    "description": "Force send notification, when set to true will send notification even if alert threshold is not reached, defaults to false",
                    "type": "boolean",
                    "example": false
                },
                "providers": {
                    "description": "List of cloud providers to analyze, supports: volcengine (VolcEngine), alicloud (Alibaba Cloud), aws, azure, gcp, defaults to [\"volcengine\", \"alicloud\"]",
                    "type": "array",
                    "items": {
                        "type": "string"
                    },
                ...
                "test_mode": {
                    "description": "Test mode, when set to true only tests WeChat Work connection without sending actual cost report, defaults to false",
                    "type": "boolean",
                    "example": false
                }
            }
        ...
        "models.WeChatNotificationStatusResponse": {
            ...
            "properties": {
                ...
                "enabled": {
                    "description": "Whether WeChat notification feature is enabled",
                    "type": "boolean",
                    "example": true
                },
                "executor_enabled": {
                    "description": "Whether executor is enabled",
                    "type": "boolean",
                    "example": true
                },
                "executor_status": {
                ...
                "retry_delay": {
                    "description": "Retry delay time (seconds)",
                    "type": "integer",
                    "example": 5
                },
                "timestamp": {
                    "description": "Status query timestamp",
                    "type": "string",
                    "example": "2025-09-12T08:13:24Z"
                },
                "webhook_configured": {
        ...
        "models.WeChatTestRequest": {
            ...
            "properties": {
                "custom_message": {
                    "description": "Custom test message content, maximum length 500 characters, defaults to system preset test message",
                    "type": "string",
                    "maxLength": 500,
                    "example": "This is a test message from cost monitoring system"
                },
                "timeout": {
                    "description": "Connection timeout (seconds), range 1-60 seconds, defaults to 10 seconds",
                    "type": "integer",
                    "maximum": 60,
                    "minimum": 1,
                    "example": 10
...
var SwaggerInfo = &swag.Spec{
	...
	Title:            "Goscan API",
	Description:      "Cloud billing data synchronization service API documentation\nSupports automatic synchronization of billing data from multi-cloud platforms like VolcEngine and Alibaba Cloud to ClickHouse database",
